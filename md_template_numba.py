...
class MDsimulator:
    ...
    def propagate(self) :
        ...
        for i in range(0,self.n):
            ...
            # THERMOSTAT IS INSERTED HERE RETARD
            md.thermalize(self.vx, self.vy, np.sqrt(self.kBT/self.mass))   # remove this line if you want no thermostats
    ...
    def plot_energy(self, title="energies") :
        ...
        plt.figure()
        plt.title(f'thermalised system, $\sigma = \epsilon = 1$, T = {self.T} and dt = {self.dt}')
        plt.xlabel('time')
        plt.ylabel('energy')
        plt.plot(self.outt, self.ekinList, self.outt, self.epotList, self.outt, self.etotList)
        plt.legend( ('Ekin','Epot','Etot') )
        plt.savefig(f'thermalisedseT{self.T}dt{self.dt}' + ".png")
        plt.show()
    ...
    def average_pressure(self):
        ...
        """
            Computes the average pressure of the system
        """
...
def exercise_32D():
    ...
    for T in T_list: 
        ...
        hc = obs.average_heat_capacity()
        ae = obs.average_energy()

        Heat_caps.append(hc)
        Average_energy.append(ae)

    plt.figure()
    plt.title(f'Heatcapacities plotted as a function of Temperature ( dt = {obs.dt})')
    plt.xlabel('Temperature')
    plt.ylabel('Heatcapacity')
    plt.plot(T_list, Heat_caps, label='Heatcapacity')
    plt.plot(T_list, Average_energy, label='Average energy')
    plt.legend()
    plt.savefig(f'heatcapsandenergiesdt{obs.dt}' + ".png")
    plt.show()
...
def exercise_32e():
    ...
    for T in T_list: 
        obs1 = MDsimulator(n = 48,   mass = 1.0,  numPerRow = 8, initial_spacing = 1.12, T = T, dt = 0.01, nsteps = 50000, numStepsPerFrame = 100,startStepForAveraging = 100)
        obs1.simulate()
        obs2 = MDsimulator(n = 48,   mass = 1.0,  numPerRow = 8, initial_spacing = 2.24, T = T, dt = 0.01, nsteps = 50000, numStepsPerFrame = 100,startStepForAveraging = 100)
        obs2.simulate()

        #ap1 = obs1.average_pressure()
        #ap2 = obs2.average_pressure()

        Average_pressure_l1.append(obs1.P)
        Average_pressure_l2.append(obs2.P)


    plt.figure()
    plt.title(f'Pressures over Temperature and with different initial spacings ( dt = {obs1.dt})')
    plt.xlabel('Temperature')
    plt.ylabel('Pressure')
    plt.plot(T_list, Average_pressure_l1, label='pressure with initial spacing = 1.12')
    plt.plot(T_list, Average_pressure_l2, label='pressure with initial spacing = 2.24')
    plt.legend()
    plt.savefig(f'pressuresdt{obs1.dt}' + ".png")
    plt.show()
